                ...
                <div className="absolute z-10 mt-1 w-48 origin-top-right rounded-md bg-white border border-gray-200 shadow-lg focus:outline-none">
                    {options.map((option) => (
                        <button
                            ...
                            className={`flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-700 hover:bg-indigo-100 focus:bg-indigo-100 ${selected.locale === option.locale ? "bg-indigo-200 font-semibold" : ""
                                }`}
                        >
                            {option.icon}
                            {option.label}
